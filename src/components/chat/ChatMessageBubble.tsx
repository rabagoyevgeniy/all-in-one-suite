...
import { Download, FileText, Play, Pause, Forward } from 'lucide-react';
  ...
  try {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === query.toLowerCase()
        ? <mark key={i} className="bg-[hsl(48_96%_70%)] rounded px-0.5">{part}</mark>
        : part
    );
  } catch {
    return text;
...
function VoiceBubble({ url, isOwn }: { url: string; isOwn: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1 min-w-[160px]" onClick={e => e.stopPropagation()}>
...
export default function ChatMessageBubble({ msg, isOwn, showName, otherLastRead, isDirect, onNameClick, searchQuery }: ChatMessageBubbleProps) {
      ...
      <>
        <div className="relative cursor-pointer" onClick={() => setLightboxOpen(true)}>
          <img
            ...
            className={cn('max-w-[240px] rounded-2xl object-cover', isOwn ? 'rounded-tr-sm' : 'rounded-tl-sm')}
            loading="lazy"
          />
        ...
        <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}>
          <DialogContent className="max-w-[90vw] max-h-[90vh] p-1 bg-[hsl(0_0%_0%/0.9)] border-none">
            <img src={msg.media_url} alt={msg.media_name || 'Image'} className="w-full h-full object-contain rounded" />
          </DialogContent>
        </Dialog>
      </>
    );
  ...
  return (
    <>
      ...
      >
        {showName && (
          <p
            className="text-[10px] font-semibold mb-0.5 text-primary cursor-pointer hover:underline"
            onClick={(e) => { e.stopPropagation(); onNameClick?.(msg.sender_id); }}
          >
        ...
        {msg.forwarded_from_name && (
          <div className={cn(
            ...
            isOwn ? 'text-[hsl(0_0%_100%/0.6)]' : 'text-muted-foreground'
          )}>
            <Forward className="w-3 h-3" />
            <span>Forwarded from {msg.forwarded_from_name}</span>
          </div>
        )}
        ...
        {replyMessage && (
          <div className={cn(
            ...
            <p className="font-semibold opacity-70 truncate">↩ {replyMessage.sender?.full_name || ''}</p>
        ...
        {messageType === 'image' && msg.media_url && (
          <div className="mb-1.5">
            <img
              src={msg.media_url} alt={msg.media_name || 'Image'}
              className="max-w-[240px] rounded-xl cursor-pointer object-cover"
              onClick={() => setLightboxOpen(true)} loading="lazy"
            />
          </div>
        )}
        ...
        {messageType !== 'voice' && !(messageType === 'image' && msg.body === msg.media_name) && (
          <p className="whitespace-pre-wrap break-words">{highlightText(msg.body, searchQuery)}</p>
        )}

        <div className="flex items-center justify-end gap-1 mt-0.5">
          {msg.is_edited && (
            <span className={cn('text-[9px]', isOwn ? 'text-[hsl(0_0%_100%/0.5)]' : 'text-muted-foreground')}>edited</span>
          )}
          <p className={cn('text-[9px]', isOwn ? 'text-[hsl(0_0%_100%/0.6)]' : 'text-muted-foreground')}>{time}</p>
          {isDirect && isOwn && (
